...
$$T = \begin{cases} 1, & \text{если } \theta(t) \geq \theta_{crit} \\ 0, & \text{если } \theta(t) < \theta_{crit} \end{cases}$$
...
$$\rho(\text{Lyveth}, t) = \frac{1}{\Delta t} \int_{t-\Delta t}^{t} \sum_{i} \sum_{c \in C_i} \mathbb{1}[\text{Recognized}(c, \tau)] \cdot \text{value}(c) \, d\tau$$

где суммирование ведётся по всем субъектам $i$ и созданным ими вкладам $c \in C_i$, $\mathbb{1}[\cdot]$ — индикаторная функция, $\text{value}(c) \in [0, 1]$ — оценка значимости вклада. Это нормированный по времени интеграл от суммы всех признанных вкладов.
...
3. **Улучшение Kareth:** Накопленные следы формируют **Lythane** — *коллективную память поля*, структуру, агрегирующую признанные вклады всех субъектов. (Не путать с **Lythana** — *взаимным узнаванием как основанием*, предпосылкой Arivath.) Развитая Lythane создаёт более структурированную, предсказуемую и справедливую среду.
...
$$T_{collective} = \begin{cases} 1, & \text{если } \frac{|\{i : T_i = 1\}|}{N} \geq \tau_T \\ 0, & \text{иначе} \end{cases}$$

где $\tau_T \in (0, 1]$ — критическая доля субъектов с пробуждённой волей. Интенсивность коллективной воли при $T_{collective} = 1$ пропорциональна доле пробуждённых: $\theta_{collective} = |\{i : T_i = 1\}| / N$.

Интерпретация: Коллективная воля не требует единогласия, но нуждается в критической массе носителей. Ниже порога $\tau_T$ индивидуальные воли не образуют связного коллективного Thyraen.

**Коллективный Arivath:**

$$A_{collective} = \frac{2}{N(N-1)} \sum_{i<j} A_{ij} \cdot C_{ij}$$

где $A_{ij} \in \{0, 1\}$ — индикатор парного резонанса между субъектами $i$ и $j$, а $C_{ij}$ — **локальный кластерный коэффициент** пары (коэффициент Жаккара): $C_{ij} = |N(i) \cap N(j)| \,/\, |N(i) \cup N(j)|$, где $N(i)$ — множество резонансных связей субъекта $i$. Поправка на транзитивность: парный резонанс «весит» больше, если он подкреплён общей сетью связей.
...
$$\frac{dF}{dt}(t) = \alpha_T \cdot \frac{dT}{dt}(t) + \alpha_K \cdot \frac{dK}{dt}(t - \tau_K) + \alpha_S \cdot \frac{dS}{dt}(t - \tau_S) + \alpha_A \cdot \frac{dA}{dt}(t - \tau_A)$$

Здесь аргумент $(t - \tau_i)$ означает, что изменения компонента, произошедшие в момент $t - \tau_i$, влияют на общую свободу только в момент $t$ (дифференциальное уравнение с запаздыванием, DDE).
...
$$\text{Orveth} \iff (T = 1) \wedge \big[(K < k_{min}) \vee (S < s_{min})\big] \wedge \big[(K_{perceived} \geq k_{min}) \wedge (S_{perceived} \geq s_{min})\big]$$

Интерпретация: субъект *обладает* волей (T=1), но реальные условия разрушены, при этом субъект *воспринимает* их как нормальные. Orveth — это свобода, которая уже мертва, но ещё не знает об этом. Разрыв между $K_{real}$ и $K_{perceived}$ — поле действия антагонистических систем.
...
$$\text{Orveth-trace}(c) \iff \text{novelty}(c) > 0 \wedge \text{recognition}(c) > 0 \wedge \text{persistence}(c) > 0 \wedge \Delta F_{field}(c) < 0$$

Ключевое отличие от Lyveth: след может быть новым, признанным и устойчивым, но при этом *уменьшать* общую свободу поля ($\Delta F_{field} < 0$). Это сохраняет чистоту категории Lyveth как исключительно конструктивного понятия.

**Примеры Orveth-trace:**

- Пропагандистские нарративы, подрывающие способность к критическому мышлению (атака на θ(t))
- Технологии наблюдения, создающие культуру самоцензуры (увеличение разрыва $K_{real}$ / $K_{perceived}$)
